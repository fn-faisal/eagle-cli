...
var CmdScreen = /** @class */ (function () {
    ...
    CmdScreen.prototype.execute = function (name) {
        ...
        // set up routing.
        var routes = require(PATH_SRC + path.sep + 'routes.ts')["default"];
        if (Object.keys(routes).length <= 0) {
            routes['/'] = CapitalCase;
        }
        else
            routes[CapitalCase.toLocaleLowerCase()] = CapitalCase;
        fs.writeFile(PATH_SRC + path.sep + 'routes.ts', "export default " + JSON.stringify(routes), 'utf8', function (err) {
            if (err)
                return console.log(err);
        });
    };
